...
def flatten_json_schema(schema: Dict[str, Any]) -> Dict[str, Any]:  # noqa: C901
    ...
    def resolve(node: Any) -> Any:
        if isinstance(node, dict):
            ...
            # Recurse into dict
            filtered = {
                k: v for k, v in node.items() if k in allowed_keys or k not in unsupported_keys
            }
...
def _extract_json_object(text: str) -> Optional[str]:  # noqa: C901
...
def evaluate_batch_with_gemini(
    pairs: List[Tuple[str, str]], model_name: str = "gemini-2.5-flash", temperature: float = 0.5
) -> Tuple[str, Dict[str, Optional[int]], str]:
    """
...
def extract_creativity_score(text: str) -> Optional[int]:
    ...
    """
    match = re.search(
        r"Creativity(?:/Innovation)? Score[:\s-]*\*?\*?\s*(\d{1,2})", text, re.IGNORECASE
    )
    if match:
        score = int(match.group(1))
        if 1 <= score <= 10:
